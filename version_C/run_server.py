...
def resolve_bind_host() -> str:
    """로그인·RBAC이 있어도 기본은 로컬 루프백 바인딩을 강제한다.

    `SETUK_BIND_HOST`를 루프백이 아닌 주소로 설정하려면
    `SETUK_ALLOW_NON_LOOPBACK=1`을 함께 켜야 한다 — 실수로 `0.0.0.0`으로
    바꾸거나 컨테이너에서 포트를 그대로 노출해도 여기서 막힌다.
    """
    ...
    if os.environ.get("SETUK_ALLOW_NON_LOOPBACK", "").strip() in {"1", "true", "TRUE", "yes"}:
        return host
    raise RuntimeError(
        f"SETUK_BIND_HOST={host!r}는 루프백 주소가 아닙니다. 학교 네트워크 등에 "
        "실제로 열 계획이면 HTTPS(리버스 프록시)까지 준비한 뒤 "
        "SETUK_ALLOW_NON_LOOPBACK=1로 명시적으로 허용하세요."
    )
