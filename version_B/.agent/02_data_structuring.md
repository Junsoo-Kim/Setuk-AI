...
> 파이프라인상 역할: 리서치·구조화 에이전트. (기획서.md 6.2의 역할 분리 원칙 참고 — 문장을 쓰거나 평가하지 않는다.)
...
8. 입력 YAML의 `activities[]` 항목에 `priority`·`source_report`·`role` 등 다중 보고서 모드(창체 C모드, 물리 통합 세특 E모드)가 붙인 라우팅용 메타데이터가 있으면, 값을 바꾸지 않고 대응하는 `STRUCTURED_FACTS_V1.activities[]` 항목에 그대로 옮긴다. 단일 보고서 세특(A·B모드) 입력처럼 이런 필드가 없으면 아무것도 추가하지 않는다.
...
아래 필드를 사용한다. `proposition`은 반드시 완결된 문장형 명제로 작성한다. 위 처리 절차 8번에 따라 입력에 `priority`·`source_report`·`role`이 있으면 해당 `activities[]` 항목에 그대로 옮겨 적는다(단일 보고서 입력에는 없으므로 추가하지 않는다).
